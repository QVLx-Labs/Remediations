...
class BlueNoroffRemediation {
    ...
    func scanForIoCs() {
        ...
        for directory in directoriesToScan {
            let fileURLs = FileManager.default.contentsOfDirectory(at: URL(fileURLWithPath: directory), includingPropertiesForKeys: nil)
    ...
    func hashOfFile(at url: URL) -> String? {
        guard let inputStream = InputStream(url: url), let buffer = UnsafeMutablePointer<UInt8>.allocate(capacity: 1024) else {
